...
class UserPermissions
{
    ...
    const PERMISSION_MANAGE_ARTICLE = 'manage_article';
     ...
     *
     * @param Article $articlePage
     * @return bool
     */
    public static function canUpdateArticlePage(Article $wikiPage)
    {
        if (Yii::$app->user->isGuest) {
            return false;
        }

        if (Yii::$app->user->can(self::PERMISSION_MANAGE_ARTICLE)) {
            return true;
        }

        if ((int)$wikiPage->created_by === (int)Yii::$app->user->getId()) {
            return true;
        }
