...
class m150725_192740_seed_data extends Migration
{
    ...
    public function safeUp()
    {
        ...
        $arrCategory = ['Tips', 'How-tos', 'Tutorial', 'Faqs', 'Others'];
        ...
        $this->insert('{{%key_storage_item}}', [
            'key' => 'backend.dark-theme',
            'value' => 1
        ]);

        $this->insert('{{%key_storage_item}}', [
            'key' => 'backend.sidebar-lg-show',
            'value' => 1
        ]);

        $this->insert('{{%key_storage_item}}', [
            'key' => 'backend.sidebar-minimized',
            'value' => 0
        ]);
        ...
        foreach ($arrArticle as $key => $item) {
            $this->insert('{{%article}}', [
                ...
                'title' => $item,
                'body' => $item,
                'status' => \common\models\Article::STATUS_PUBLISHED,
                'created_at' => time()
            ]);
        }
